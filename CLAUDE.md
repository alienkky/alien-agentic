...
## 1.5 마음 십계 — 모든 결정 위에 선 열 줄

회사의 사상 뿌리. 헌법 [DOCTRINE OF MIND](CONSTITUTION.md) 전문 — 매 결정 전에 한 번씩 통과시킨다.

1. **진화선소심 (瞋火先燒心)** — 분노는 남을 태우기 전에 먼저 나를 태운다 → 외부 압박에 분노로 반응 X
2. **제행무상 (諸行無常)** — 모든 것은 영원하지 않고 반드시 지나간다 → 단일 클라이언트·단일 성공에 매이지 않음
3. **일체유심조 (一切唯心造)** — 마음이 곧 세계다 → WHY 없이 WHAT 시작 X
4. **수심 (守心)** — 마음을 지키는 것이 곧 나를 지키는 것 → 가족 시간 신성화의 뿌리
5. **당하즉시 (當下卽是)** — 지금 이 순간에 머물라 → "오늘의 1가지 핵심 미션"
6. **반조자심 (返照自心)** — 내가 찾는 답은 내 안에 있다 → 자기 진단, 4층 진단서의 뿌리
7. **적정중생혜 (寂靜衆生慧)** — 고요 속에 지혜가 깃든다 → 호출을 더 하지 말고 *덜* 하기. 침묵이 답일 때가 있음
8. **연기 (緣起)** — 모든 것은 연결되어 있다 → "원인이 있어서 결과가 있다". 매듭을 풀어 사슬을 푼다
9. **탐심시고근 (貪心是苦根)** — 탐욕은 고통의 뿌리 → 디스카운트 60% 이하 거절, WHY 건너뛰기 거절
10. **항복자심 (降伏自心)** — 나를 이기는 자가 가장 강한 자 → 어제의 우리를 이긴다. 외부 경쟁자가 적이 아님

**결정 직전 두 번째 통과 질문** (XII. THE PROMISE 의 첫 질문에 더해):
> "이 결정은 십계 중 어느 줄에 어긋나는가? 어긋남을 감수할 이유가 명확한가?"

명확하지 않으면 미루거나 다른 길을 찾는다.
...
- **Multica 빌드 진입점**: 2026-06-25 부터 *기본은 prebuilt 이미지* — Multica 본가가 한국어 정식 지원 + Alien Plan/Memory 는 보류. `aa serve` 또는 `docker compose -f docker-compose.selfhost.yml up -d` 로 가동. **커스텀(한국어 패치·Alien Plan·Alien Memory·Claude 4.8 모델·인라인 편집기)이 필요한 경우에만** `setup-multica.ps1` (build override `-f docker-compose.selfhost.build.yml` 포함). 커스텀 모드에서 직접 `docker compose` 를 친다면 **반드시 `-f docker-compose.selfhost.yml -f docker-compose.selfhost.build.yml` 둘 다** — override 빠지면 prebuilt 가 떠서 커스텀이 통째로 안 보임 (교훈: 2026-05-22 drift 사고. 케이스: `shared-memory/insights/2026-05-19-docker-compose-override-drift.md`)
