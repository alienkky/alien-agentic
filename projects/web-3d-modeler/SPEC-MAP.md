...
| 3.2 CurveEntity | Line/Arc | ✅ | 선/사각형/원/타원/다각형/**호(3점)/스플라인(Catmull-Rom)** + 자르기·삭제 |
...
| **Shapr3D 선택 모델** | ✅ | 단일탭=면/모서리, **더블탭=바디**, 추가탭=다중(토글), 빈곳탭=해제 |
...
**스케치 도구 남은 것:** 텍스트 · 이동/회전 · 구속조건 솔버(Module 7) _(완료: 선·사각형·원·타원·다각형·호·스플라인·자르기·삭제·**미러·패턴·오프셋·투상**. 다음: 이동/회전·텍스트 → 점 드래그 편집)_
