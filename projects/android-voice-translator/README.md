...
- **폰 내부 소리 자막** — 폰에서 재생되는 영상·앱 소리를 직접 캡처해 번역(`🔊` 버튼). Android 10+ · MediaProjection 권한 · 클라우드 Whisper 필요. (일부 DRM/스트리밍 앱은 캡처를 차단)
- **TTS 음성 출력** — 번역문을 소리로 읽어줌(목표 언어로). 설정 토글.
- **오버레이 자막** — 다른 앱 위에 떠 있는 드래그 가능한 실시간 자막(포그라운드 서비스). `⧉ 오버레이`. 오버레이의 **A− / A+** 로 글자 크기를 즉시 조절, **✕ 닫기**로 종료.
- **오버레이 모양 설정** — 설정에서 **글자 크기**(16~48sp)와 **배경 투명도**(20~100%) 조절.
- **번역 결과 언어 선택** — 한국어 / 영어 / 일본어 중 택1(`⚙ 설정`). 기본 **한국어**. 들린 언어를 자동 감지해 선택한 언어로 번역(예: 일본어 음성 → 한국어 자막).
- **듣기 언어 토글** — 日本語 / English / 한국어 순환(STT 인식 정확도용).
...
- 클라우드 Whisper는 발화 단위 전송이라 약간의 지연이 있습니다(정확도↔지연 트레이드오프). 실시간 스트리밍 STT로 더 줄일 수 있습니다.
  - **속도 팁**: 전송이 녹음을 막지 않도록 별도 스레드 처리, 침묵 컷 0.5초, 긴 발화는 ~6초마다 분할. OpenAI라면 설정의 모델을 `gpt-4o-mini-transcribe`(whisper-1보다 빠름)로 바꾸면 더 빨라집니다.
